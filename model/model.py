...
def _update_index_matrix(
    ...
    if index_matrix is None or index_matrix.shape[0] != batch_size * 2:
        index_matrix = F.zeros([2 * batch_size, 2 * batch_size])
        if linear:
...
class GaussianKernel(nn.Module):
    ...
    def forward(self, X):
        ...
        # megengine 适配
        l2_distance_square = ((F.expand_dims(X, 0) - F.expand_dims(X, 1)) ** 2).sum(2)

        if self.track_running_stats:
            self.sigma_square = self.alpha * F.mean(l2_distance_square.detach())

        return F.exp(-l2_distance_square / (2 * self.sigma_square))
